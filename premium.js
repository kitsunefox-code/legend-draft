...
cardHtml=function(p,rank,opt={}){
  ...
  return `<div class="pc ld-card r-${rank} sz-${size} ${opt.cls||''} ${photo.src?(photo.hi?'ld-hires':'ld-low-res'):'ld-no-photo'}" style="--club:${club}" aria-label="${attr(p.name+'・'+rank+'ランク・'+(opt.pos||roleLabel(p)))}" title="${attr(p.name)}" ${action}>
    <div class="pc-in">
      ...
      <div class="ld-card-top"><div class="ld-card-left"><div class="ld-card-rk">${p.danger?dokuroSvg(size==='l'?54:size==='m'?34:22):rankIcon(rating,size==='l'?54:size==='m'?34:22)}</div>${typeof marksHtml==='function'?marksHtml(p,size==='l'?24:size==='m'?17:12):''}</div><span class="ld-card-posn">${esc(opt.pos||roleLabel(p))}</span></div>
