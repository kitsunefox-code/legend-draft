...
function ldParkStage(x){
  ...
  return '<section class="home-ceremony home-'+phase+(mapHtml?' home-mapping':'')+(era?' home-retro':'')+'" style="--rk:'+tint+'" aria-label="本拠地の決定">'+mapHtml+'<img class="home-panorama" src="'+(photo||'stadium.webp')+'" alt="'+(done&&photo?esc(pk.name):'')+'" onerror="this.src=\'stadium.webp\';this.onerror=null;this.closest(\'section\').classList.add(\'home-fallback\')"><img class="home-tunnel" src="stadium-entry.webp" alt=""><div class="home-shade"></div><div class="home-light" aria-hidden="true"></div><div class="home-lamps" aria-hidden="true"><i></i><i></i><i></i><i></i></div><div class="home-content">'+(done?'<span class="home-eyebrow">'+esc(gachaTeam().name)+' の本拠地<i>'+esc(PARK_CITY[pk.id]||'')+'</i></span><h2>'+esc(pk.name)+'</h2>'+(era?'<p class="home-era"><b>'+era.y+'年'+(pk.cat==='MLB'?'':'・'+ldWareki(era.y))+'</b>'+esc(era.n)+'</p>':'')+'<p class="home-tagline">'+ldParkTagline(pk)+'</p><div class="home-info"><div class="home-facts"><span><small>球場の格</small><b class="home-grade">'+parkRank(pk)+'</b></span><span><small>本塁打</small><b>'+(pk.hr>=1.12?'出やすい':pk.hr<=.9?'出にくい':'標準')+'</b></span><span><small>傾向</small><b>'+(pk.run>=1.06?'打者有利':pk.run<=.96?'投手有利':'バランス')+'</b></span></div><p class="home-type">'+esc(pk.cat)+' ／ '+esc(pk.type)+'</p><div class="home-traits">'+parkTraitChips(pk)+'</div><p class="home-note">'+esc(pk.note||'')+'</p></div>':'<span class="home-eyebrow">本拠地を決める</span><h2>'+(phase==='idle'?'ここから、<br>球団の歴史が始まる。':phase==='approach'?'スタンドの、その先へ。':'照明が、灯る。')+'</h2><p class="home-type">'+(phase==='idle'?'まだ誰もいない球場へ。':'まもなく、あなたのホームが決まります。')+'</p>')+'</div><button type="button" class="ceremony-tap-surface" aria-label="'+(done?'本拠地を確認して次へ':'タップして球場へ入る')+'" onclick="ldCeremonyTap()"></button><div class="ceremony-tap-note">'+(done?(ldDemo?'タップして別の球場へ':'タップして次へ'):phase==='idle'?'タップして球場に入る':'')+'</div>'+(done?'<div class="home-credit">'+(credit?'<a href="'+esc(credit.u)+'" target="_blank" rel="noopener">写真：'+esc(credit.a)+' / '+esc(credit.l)+'</a>':'<span>球場イメージ</span>')+'<span class="home-image-fallback">球場イメージ</span></div>':'')+'</section>';
}
