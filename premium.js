...
cardHtml=function(p,rank,opt={}){
  ...
  return `<div class="pc ld-card r-${rank} sz-${size} ${opt.cls||''} ${photo.src?(photo.hi?'ld-hires':'ld-low-res'):'ld-no-photo'}" style="--club:${club}" aria-label="${attr(p.name+'・'+rank+'ランク・'+(opt.pos||roleLabel(p)))}" title="${attr(p.name)}" ${action}>
    <div class="pc-in">
      ...
      <div class="ld-card-top"><div class="ld-card-left"><div class="ld-card-rk">${p.danger?dokuroSvg(size==='l'?54:size==='m'?34:22):rankIcon((rank&&rank!=='D')?{rank:rank,ovr:rating}:rating,size==='l'?54:size==='m'?34:22)}</div>${typeof marksHtml==='function'?marksHtml(p,size==='l'?24:size==='m'?17:12):''}</div><span class="ld-card-posn">${esc(opt.pos||roleLabel(p))}</span></div>
...
gachaReveal=function(i){
  ...
  let shown=false;ldFinish=()=>{if(shown||state.gacha!==G)return;shown=true;overlay.classList.add('revealed');if(x.rank==='SS')seFanfare();else seWin();overlay.querySelector('.ld-reveal-result button').focus();if(ldAuto)ldDelay(()=>{if(ldAuto)ldAccept();},rare?1900:700);else if(!rare&&(kind==='B'||kind==='P'))ldDelay(()=>{if(document.querySelector('.ld-reveal')===overlay&&overlay.classList.contains('revealed')&&!x.open)overlay._accept();},1000);};
