...
function ldField(){
  ...
  if(R.k==='B'){
    const field=slots.map((x,i)=>GC_FIELD[x.d.key]?'<div class="gc-at" style="left:'+GC_FIELD[x.d.key][0]+'%;top:'+GC_FIELD[x.d.key][1]+'%;--dx:'+(50-GC_FIELD[x.d.key][0])+'cqw;--dy:'+(112-GC_FIELD[x.d.key][1])+'cqh">'+ldSlot(x,i)+'</div>':'').join('');
  ...
  const x=slots[0];return '<div class="ld-single"><div class="ld-single-caption">'+(R.k==='K'?'本拠地':R.k==='C'?'マスコット':'監督')+'</div>'+(x?(x.open?gachaCardHtml(x,true):'<div class="gc-spot one" data-i="0">'+gachaCapHtml(x,0,true)+'</div>'):'')+'<p>'+(R.k==='K'?'この球場から、伝説が始まる。':R.k==='C'?'球団の顔。特殊能力がシーズンに効く。':'あなたの球団を率いる、ひとり。')+'</p></div>';
}
...
cardHtml=function(p,rank,opt={}){
  ...
  return `<div class="pc ld-card r-${rank} sz-${size} ${opt.cls||''} ${photo.src?(photo.hi?'ld-hires':'ld-low-res'):'ld-no-photo'}" style="--club:${club}" data-pid="${attr(String(p.id??''))}" aria-label="${attr(p.name+'・'+rank+'ランク・'+(opt.pos||roleLabel(p)))}" title="${attr(p.name)}" ${action}>
    <div class="pc-in">
      ...
      <div class="ld-card-top"><div class="ld-card-left"><div class="ld-card-rk">${p.danger?dokuroSvg(size==='l'?54:size==='m'?34:22):rankIcon((rank&&rank!=='D')?{rank:rank,ovr:rating}:rating,size==='l'?54:size==='m'?34:22)}<b>${rating}<small>OVR</small></b></div>${typeof marksHtml==='function'?marksHtml(p,size==='l'?24:size==='m'?17:12):''}</div><span class="ld-card-posn">${esc(opt.pos||roleLabel(p))}</span></div>
...
gachaReveal=function(i){
  ...
  const special=kind==='K'?'<div class="ld-special-intro venue"><div class="ld-scoreboard"><small>NEXT HOME</small><b>本拠地</b><span>本拠地、選定中</span></div><div class="ld-venue-lights"><i></i><i></i><i></i><i></i><i></i></div></div>':kind==='C'?'<div class="ld-special-intro manager"><div class="ld-contract"><small>MASCOT</small><b>MASCOT DEBUT</b><span>球団の顔が、扉の向こうに</span></div><div class="ld-press-flash"><i></i><i></i><i></i><i></i></div></div>':kind==='M'?'<div class="ld-special-intro manager"><div class="ld-contract"><small>OFFICIAL</small><b>MANAGER CONTRACT</b><span>新監督との契約を締結</span></div><div class="ld-press-flash"><i></i><i></i><i></i><i></i></div></div>':'';
  overlay.innerHTML='<button type="button" class="ld-skip" onclick="ldSkipReveal()">演出スキップ ≫</button>'+ldRevealFx()+'<div class="ld-reveal-aura"></div>'+special+ldCapsuleHero(x)+'<div class="ld-reveal-premonition"><small>'+(x.rank==='SS'?'球史をたどる':'スター選手、登場')+'</small><b>'+x.rank+'</b><span>'+(x.rank==='SS'?esc(headline):'白銀のスター、降臨')+'</span></div><div class="ld-reveal-result"><div class="ld-reveal-kicker">'+(kind==='K'?'WELCOME TO OUR BALLPARK':kind==='C'?'MASCOT DEBUT':kind==='M'?'NEW FIELD BOSS':x.rank==='SS'?'IMMORTAL LEGEND':x.rank==='S'?'Sランク選手':'新加入選手')+'</div>'+(x.p?cardHtml(x.p,x.rank,{size:'l',pos:ldPositions[x.d.key]||x.d.label,grp:x.d.grp}):gachaCardHtml(x,true))+'<div class="ld-reveal-caption">'+(x.p?esc(x.p.desc||''):x.mascot?esc(x.mascot.desc||''):esc(x.park.name||parkShort(x.park)))+'</div>'+ldButton('獲得する →','ldAccept()','primary')+'</div>';
  document.body.appendChild(overlay);overlay.querySelector('button').focus();seCrack();
  overlay.addEventListener('click',ev=>{if(overlay._lp)return;if(!overlay.classList.contains('revealed')){ldSkipReveal();return;}if(ev.target.closest('button'))return;if(typeof ldBusy!=='undefined'&&overlay._accept)overlay._accept();});
