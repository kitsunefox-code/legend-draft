...
for(const f of ['players.js','danger.js','marks.js','mascots.js','mascot-photo.js','data-fixes.js','party.js','ss.js','game.js','premium.js']){let s=fs.readFileSync(''+f,'utf8');if(f==='game.js')s=s.replace(/^buildFrontPage\(\);/m,'');vm.runInContext(s,ctx,{filename:f});}
...
const shell=run('gachaCapHtml({...state.gacha.pulls[0],rank:"SS"},0,false)');assert(shell.includes('cap-SS'));assert(!shell.includes('伝説の輝き'));assert(shell.includes('タップで開封'));
