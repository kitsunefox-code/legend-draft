...
const P = {
  ...
  batRest: {lean:4,  la:24,  lb:-14, ra:-22, rb:6,   lu:8,   lk:-2,  ru:-8,  rk:-2, bat:-14, batHand:"R"}, // バットを下ろし、後ろの手で体の後ろへ
  helmet:  {lean:6,  la:140, lb:92,  ra:-22, rb:6,   lu:8,   lk:-2,  ru:-8,  rk:-2, bat:-14, batHand:"R"}, // 後ろの手にバット、前の手でヘルメットの上を叩く
  square:  {gesture:"review",lean:2,  la:60,  lb:-100,ra:-60, rb:100, lu:8,   lk:-2,  ru:-8,  rk:-2},                  // 胸の前で両手の四角(リクエスト)
  // 打撃: 構え(足を開き膝を内へ、手は後ろの肩の高さ、後ろの肘は下げて畳む、バットは45度)
  ...
  // 構え: 前の腕は肘を下げて前腕を胸の前へ畳み、後ろの腕は肘を下げて後ろから手を合わせる。両手は後ろの肩の前。
  //       バットは後ろの肩の上に45度で寝かせ、先は頭の後ろ。膝は両方軽く曲げる
  bat:     {lean:8,  la:30,  lb:-160,ra:-40, rb:-172,lu:18,  lk:-12, ru:-18, rk:-12, bat:-135},       // 構え
  batLoad: {lean:6,  la:20,  lb:-160,ra:-55, rb:-168,lu:8,   lk:-16, ru:-22, rk:-12, bat:-130},       // 割れ(手は少し後ろへ、前の膝は内へ)
  batStride:{lean:4, la:18,  lb:-160,ra:-58, rb:-168,lu:36,  lk:-8,  ru:-22, rk:-6, bat:-130},        // 踏み込み(トップ)
  swing:   {lean:-4, la:85,  lb:-8,  ra:80,  rb:-14, lu:28,  lk:0,   ru:-22, rk:40, bat:78},          // インパクト(両腕が伸び、先は手より上)
  batFollow:{lean:-6,la:-10, lb:-120,ra:-20, rb:-120,lu:26,  lk:-4,  ru:-34, rk:46, bat:-115},        // フォロー(手は前の肩、バットは背中側へ)
...
// スイング: t はインパクトを 0 とするミリ秒(-380 から)。構え→割れ→踏み込み→加速→インパクト→フォロー
function swingPose(t){
  if(t < -380) return P.bat;
  if(t < -220) return lerpP(P.bat, P.batLoad, ease((t + 380) / 160));
  if(t < -90) return lerpP(P.batLoad, P.batStride, ease((t + 220) / 130));
  if(t < 0) return lerpP(P.batStride, P.swing, ease((t + 90) / 90));
  if(t < 240) return lerpP(P.swing, P.batFollow, ease(t / 240));
  return P.batFollow;
}
...
// 正面を向いた監督。k={lx,ly,rx,ry}=両手の位置(足元を原点、上が負)。人差し指で四角を描くための絵
function managerFront(id, x, y, sc, col, k){
  const pt = (dx, dy) => (x + dx * sc).toFixed(1) + " " + (y + dy * sc).toFixed(1);
  const w = v => (v * sc).toFixed(1);
  const line = (d, c, wd) => '<path d="' + d + '" stroke="' + c + '" stroke-width="' + w(wd) + '" fill="none" stroke-linecap="round" stroke-linejoin="round"/>';
  const arm = (sx, sy, hx, hy) => {                       // 肩→肘(脇に置く)→手。前腕で胸の前に手を運ぶ
    const ex = sx < 0 ? -14 : 14, ey = -42;
    return line('M' + pt(sx, sy) + ' L' + pt(ex, ey), col, 6.5) + line('M' + pt(ex, ey) + ' L' + pt(hx, hy), "#c98f68", 4.2) +
      '<circle cx="' + (x + hx * sc).toFixed(1) + '" cy="' + (y + hy * sc).toFixed(1) + '" r="' + w(2.6) + '" fill="#c98f68"/>' +
      line('M' + pt(hx, hy) + ' l0 ' + (-4.5 * sc).toFixed(1), "#c98f68", 1.6);   // 人差し指を立てる
  };
  // 描いた四角の線(指の軌跡)。k.q: 0〜1
  let trace = "";
  if(k.q !== undefined && k.half){
    const top = k.top, bot = k.bot, half = k.half, L = half + (bot - top) + half;
    const len = k.q < .25 ? half * (k.q / .25) : k.q < .6 ? half + (bot - top) * ((k.q - .25) / .35) : k.q < .85 ? half + (bot - top) + half * ((k.q - .6) / .25) : L;
    const path = side => 'M' + pt(0, top) + ' L' + pt(side * half, top) + ' L' + pt(side * half, bot) + ' L' + pt(0, bot);
    trace = [1, -1].map(side => '<path d="' + path(side) + '" fill="none" stroke="#fff" stroke-opacity=".55" stroke-width="' + w(1.4) + '" stroke-dasharray="' + (len * sc).toFixed(1) + ' 999"/>').join("");
  }
  return '<g id="' + id + '"><ellipse cx="' + x.toFixed(1) + '" cy="' + (y + 1).toFixed(1) + '" rx="' + w(13) + '" ry="' + w(3) + '" fill="#09130e" opacity=".24"/>' +
    line('M' + pt(-6, -30) + ' L' + pt(-8, 0), "#e5e2d8", 7.5) + line('M' + pt(6, -30) + ' L' + pt(8, 0), "#e5e2d8", 7.5) +
    line('M' + pt(-11, 1) + ' h7 M' + pt(4, 1) + ' h7', "#151c23", 3) +
    '<path d="M' + pt(-13, -31) + ' L' + pt(-13, -55) + ' Q' + pt(0, -60) + ' ' + pt(13, -55) + ' L' + pt(13, -31) + ' Z" fill="' + col + '" stroke="' + shade(col, .64) + '" stroke-width=".8"/>' +
    trace + arm(-11, -53, k.lx, k.ly) + arm(11, -53, k.rx, k.ry) +
    '<circle cx="' + x.toFixed(1) + '" cy="' + (y - 65 * sc).toFixed(1) + '" r="' + w(7) + '" fill="#c98f68"/>' +
    '<path d="M' + pt(-7.5, -66) + ' a7.5 7.5 0 0 1 15 0 l1 2 h-17 z" fill="#111"/>' +
    '</g>';
}
// 申請の所作を舞台の上に重ねる
function gesture(c, done){
  ...
  if(c.kind === "abs"){
    ...
    const lefty=c.lefty, bx=lefty?150:262;
    g.innerHTML=batter3d("rv-gfig",B3_KF.stride,bx,288,52,col,!lefty);svg.appendChild(g);
    const old=$r("rv-bat");if(old)old.setAttribute("opacity",0);
    let start=0;
    function f(ts){if(!start)start=ts;const t=ts-start;
      // 見逃しなのでバットは振らない: バットを後ろの手で下げて、前の手でヘルメットの上を2回叩く
      const q=b3Lerp(B3_KF.stride,B3_KF.helmet,ease(Math.min(1,t/450)));
      if(t>650) q.handL=[q.handL[0],q.handL[1]+Math.sin(Math.min(1,(t-650)/300)*Math.PI*4)*.04,q.handL[2]];
      const e=$r("rv-gfig"); if(e) e.outerHTML=batter3d("rv-gfig",q,bx,288,52,col,!lefty);
      if(t<950)RV.raf=rvFrame(f);else RV.timer=rvLater(done,180);
    }
    RV.raf=rvFrame(f);
  }else{
    // 監督がベンチから小走りで審判の前へ → 正面を向き、両手の人差し指で長方形(スクリーンサイン)を描く
    //   NPBの流儀: 胸の前で指を合わせ、横に開いて上辺、下ろして両辺、内へ寄せて下辺
    g.innerHTML = figSvg("rv-gfig", P.stand, 0, 0, 1, 1, col, "#111");
    svg.appendChild(g);
    const H = Number(svg.getAttribute("viewBox").split(" ")[3]);
    const MX = 72, MY = H - 6, MS = 1.05;
    const box = q => {                                     // q: 0〜1 で四角を一周(胸の前、肩幅の内側)
      const top = -52, bot = -38, half = 10, o = {q, top, bot, half};
      if(q < .25){ const u = q / .25; return Object.assign(o, {lx: -half * u, ly: top, rx: half * u, ry: top}); }
      if(q < .6){ const u = (q - .25) / .35; return Object.assign(o, {lx: -half, ly: top + (bot - top) * u, rx: half, ry: top + (bot - top) * u}); }
      if(q < .85){ const u = (q - .6) / .25; return Object.assign(o, {lx: -half * (1 - u), ly: bot, rx: half * (1 - u), ry: bot}); }
      return Object.assign(o, {lx: 0, ly: bot, rx: 0, ry: bot});
    };
    let start = 0;
    function f(ts){ if(!RV) return; if(!start) start = ts; const t = (ts - start);
      if(t < 700){ const p = ease(t / 700); setFig("rv-gfig", run(t / 105, 0.6), -20 + (MX + 20) * p, MY, MS, 1, col, "#111"); }
      else {
        const el = $r("rv-gfig"); if(!el) return;
        let k;
        if(t < 1000){ const u = ease((t - 700) / 300); k = {lx: -9 + 9 * u, ly: -22 - 30 * u, rx: 9 - 9 * u, ry: -22 - 30 * u}; }   // 手を胸の前へ
        else k = box(Math.min(1, (t - 1000) / 1000));
        el.outerHTML = managerFront("rv-gfig", MX, MY, MS, col, k);
      }
      if(t < 2150) RV.raf = rvFrame(f); else {
...
function zoomIn(fx, fy){
  ...
  const k = 2.8, R = 62, zx = fx > 180 ? 80 : 280, zy = 172;
  ...
  g.innerHTML = '<clipPath id="rvZc"><circle cx="' + zx + '" cy="' + zy + '" r="' + R + '"/></clipPath>' +
    ...
    '<text x="' + zx + '" y="' + (zy - R - 6) + '" text-anchor="middle" font-family="Oswald,sans-serif" font-size="9" fill="#ffd257" letter-spacing="2">CLOSE-UP ×2.8</text>';
  fadeIn(g, 320);
  svg.appendChild(g);
}
...
SCENES.home = (function(){
  ...
  const D = DIAMOND.plate, CT = 330, RUN0 = CT + 60, RUNT = 2150, HIT = CT + 620, THROW = CT + 950;   // CT=インパクト
  ...
  function draw(t, tr, c){
    ...
    // 打者: 振り抜いて一塁へ走る
    if(t < CT + 240) moveFig("rv-bat", swingPose(t - CT), BATP, 1, runCol(c), "#192a37");
    else drawRunner("rv-bat", t - CT - 240, 1650, BATP, FIRST, false, runCol(c));
    ...
    if(t < CT){ ball("rv-ball", 0, 0, 3, false); ball("rv-sh", 0, 0, 0, false); return; }
    if(t < HIT){                                                            // 打球: 本塁からライト前へ低く伸びる
      const q = (t - CT) / (HIT - CT), gx = D.home.x + (RF.x - 8 - D.home.x) * q, gy = D.home.y + (RF.y + 2 - D.home.y) * q;
...
function p3Cam(gx, gy, S){
  const C = [0.9, 2.3, 4.6], ph = 13 * Math.PI / 180, F = [0, -Math.sin(ph), -Math.cos(ph)], U = [0, Math.cos(ph), -Math.sin(ph)];
  const dot = (a, b) => a[0] * b[0] + a[1] * b[1] + a[2] * b[2];
  const o = [0 - C[0], 0 - C[1], 0 - C[2]], d0 = dot(o, F), f = S * d0;
  const ox = o[0] / d0 * f, oy = dot(o, U) / d0 * f;      // 原点(足元)の画面上の位置。これを (gx, gy) に合わせる
  return function(p){
    const d = [p[0] - C[0], p[1] - C[1], p[2] - C[2]], dep = dot(d, F);
    return {x: gx + (d[0] / dep * f - ox), y: gy - (dot(d, U) / dep * f - oy), k: d0 / dep, dep};
  };
}
// 描く。S=足元での 1m の px。col=ユニ色。rhp=右投手
function pitcher3d(id, t, gx, gy, S, col, rhp){ return pitcher3dQ(id, p3Pose(t), gx, gy, S, col, rhp); }
function pitcher3dQ(id, q, gx, gy, S, col, rhp){ return body3dQ(id, q, gx, gy, S, col, rhp, {glove:true, ball:true}); }
// 体の描画。opts: glove=左手にグラブ, ball=右手に球, bat=[x,y,z] バットの先(右手が握り)
function body3dQ(id, q, gx, gy, S, col, rhp, opts){
  opts = opts || {};
  ...
  const gl = J.handL;
  if(opts.glove) parts.push({dep: gl.dep - .02, h: '<path d="M' + (gl.x - .1 * S * gl.k).toFixed(1) + ' ' + (gl.y + .06 * S * gl.k).toFixed(1) + ' q' + (-.03 * S * gl.k).toFixed(1) + ' ' + (-.2 * S * gl.k).toFixed(1) + ' ' + (.1 * S * gl.k).toFixed(1) + ' ' + (-.2 * S * gl.k).toFixed(1) + ' q' + (.12 * S * gl.k).toFixed(1) + ' 0 ' + (.1 * S * gl.k).toFixed(1) + ' ' + (.2 * S * gl.k).toFixed(1) + ' z" fill="#8d5932" stroke="#54371f" stroke-width="1"/>'});
  else parts.push({dep: gl.dep - .02, h: '<circle cx="' + gl.x.toFixed(1) + '" cy="' + gl.y.toFixed(1) + '" r="' + (.045 * S * gl.k).toFixed(1) + '" fill="' + skin + '"/>'});
  ...
  if(opts.bat){   // バット: 右手(握り)から先へ。手前ほど太い
    const tip = pt(opts.bat), a = J.handR;
    parts.push({dep: Math.min(a.dep, tip.dep) - .03, h: '<path d="M' + a.x.toFixed(1) + ' ' + a.y.toFixed(1) + ' L' + tip.x.toFixed(1) + ' ' + tip.y.toFixed(1) + '" stroke="#b9935e" stroke-width="' + (.06 * S * (a.k + tip.k) / 2).toFixed(1) + '" stroke-linecap="round"/><path d="M' + a.x.toFixed(1) + ' ' + a.y.toFixed(1) + ' L' + (a.x + (tip.x - a.x) * .22).toFixed(1) + ' ' + (a.y + (tip.y - a.y) * .22).toFixed(1) + '" stroke="#282a29" stroke-width="' + (.045 * S * a.k).toFixed(1) + '" stroke-linecap="round"/>'});
  }
  // 頭と帽子(後ろ姿なので顔は無い)
...
// ============================================================
// 打撃フォーム(3次元)。右打者。原点は両足の中点、胸は本塁(-x)を向き、前足(左)は投手側(+z)
//   sy/hy: 肩・腰の向き。-90 = 胸が -x(構え)、-175 = 胸が投手(+z)(インパクト)
// ============================================================
const B3_KF = {
  stance:  {hip:[.05,.92,0],    sh:[.08,1.38,-.05], head:[.02,1.6,.04],   elbL:[-.26,1.14,.06], handL:[-.15,1.34,-.2],  elbR:[.06,1.12,-.36], handR:[-.13,1.32,-.22], kneeL:[.02,.5,.3],  footL:[0,0,.35],   kneeR:[.02,.5,-.3],  footR:[0,0,-.35],  hy:-90,  sy:-90,  bat:[.28,1.95,-.5]},
  load:    {hip:[.05,.92,-.08], sh:[.08,1.4,-.12],  head:[.02,1.62,-.02], elbL:[-.24,1.16,-.02],handL:[-.12,1.4,-.32],  elbR:[.1,1.15,-.46],  handR:[-.1,1.38,-.34],  kneeL:[.06,.5,.28], footL:[0,0,.35],   kneeR:[.02,.5,-.3],  footR:[0,0,-.35],  hy:-100, sy:-102, bat:[.32,1.98,-.62]},
  stride:  {hip:[.05,.9,.04],   sh:[.08,1.38,-.06], head:[.02,1.6,.03],   elbL:[-.24,1.16,0],   handL:[-.12,1.4,-.3],   elbR:[.1,1.14,-.44],  handR:[-.1,1.38,-.32],  kneeL:[.02,.5,.42], footL:[0,0,.5],    kneeR:[.02,.5,-.3],  footR:[0,0,-.35],  hy:-96,  sy:-100, bat:[.32,1.96,-.6]},
  contact: {hip:[.02,.92,.1],   sh:[0,1.35,.12],    head:[0,1.58,.14],    elbL:[-.3,1.2,.22],   handL:[-.5,1.05,.25],   elbR:[-.25,1.05,.05], handR:[-.48,1.02,.2],   kneeL:[0,.5,.5],    footL:[0,0,.5],    kneeR:[.05,.5,-.2],  footR:[.02,.1,-.3], hy:-170, sy:-175, bat:[-1.2,1.0,.38]},
  follow:  {hip:[.02,.92,.12],  sh:[0,1.35,.14],    head:[0,1.58,.2],     elbL:[-.05,1.3,.3],   handL:[.1,1.5,.35],     elbR:[.15,1.2,.2],    handR:[.08,1.48,.33],   kneeL:[0,.5,.5],    footL:[0,0,.5],    kneeR:[.06,.5,-.15], footR:[.05,.08,-.25],hy:-200, sy:-220, bat:[.6,1.7,-.2]},
  helmet:  {hip:[.05,.92,0],    sh:[.06,1.38,0],    head:[.02,1.6,.04],   elbL:[-.16,1.4,.14],  handL:[-.02,1.72,.08],  elbR:[.1,1.12,-.2],   handR:[.12,.92,-.28],   kneeL:[.02,.5,.3],  footL:[0,0,.35],   kneeR:[.02,.5,-.3],  footR:[0,0,-.35],  hy:-90,  sy:-90,  bat:[.2,.05,-.6]},   // 帽子を叩く(バットは後ろの手で下げる)
};
function b3Lerp(a, b, u){ const o = p3Lerp(a, b, u); o.bat = [0, 1, 2].map(i => a.bat[i] + (b.bat[i] - a.bat[i]) * u); return o; }
// 見逃し: t はリリースを 0 とするミリ秒。割れ→踏み込みで止める(振らない)
function b3Take(t, caught){
  const K = B3_KF;
  if(t < -330) return K.stance;
  if(t < -170) return b3Lerp(K.stance, K.load, ease((t + 330) / 160));
  if(t < 60) return b3Lerp(K.load, K.stride, ease((t + 170) / 230));
  return K.stride;
}
// スイング: t はインパクトを 0 とするミリ秒
function b3Swing(t){
  const K = B3_KF;
  if(t < -380) return K.stance;
  if(t < -220) return b3Lerp(K.stance, K.load, ease((t + 380) / 160));
  if(t < -90) return b3Lerp(K.load, K.stride, ease((t + 220) / 130));
  if(t < 0) return b3Lerp(K.stride, K.contact, ease((t + 90) / 90));
  if(t < 240) return b3Lerp(K.contact, K.follow, ease(t / 240));
  return K.follow;
}
function batter3d(id, q, gx, gy, S, col, righty){ return body3dQ(id, q, gx, gy, S, col, righty, {bat: q.bat}); }

SCENES.abs = (function(){
  const CAT = {x:200, y:268, sc:1.35};             // 捕手の足元。本塁より奥(画面では上)
  const ZB = {x:184, y:227, w:32, h:35};           // ゾーン: 打者の膝〜胸 × ベースの幅(生の場面には描かない)
  const R = 3.65 / 43.18 * ZB.w;                                     // 奥での球の大きさ
  const CM = 43.18 / ZB.w;
  const MITT0 = {x:206, y:252};                    // 構えたミットの位置
  const PIT = {x:58, y:318, S:72};                 // 投手の足元(手前・左下)。S=足元での 1m の px
  function truthOf(c){
    ...
    const Z = {x:ZB.x, y:298-height*.535*scale, w:ZB.w, h:height*(.535-.27)*scale};
  ...
  const keys=["hx","hy","sx","sy","nx","ny","kx","ky","fx","fy","bkx","bky","bfx","bfy","ex","ey","tx","ty","gx","gy","gex","gey","tw"];
  // 右投手を背中側から。x=右が投げる腕の側、y=上が負。足元が0。
  ...
  const PK = [
    ...
    [10,-33, 12,-46, 14,-54,-19,-26,-22,-20, 22,-28, 36,-34,  0,-38,-12,-26, -20,-48,-10,-46, .3],
  ...
  function pitcherBack(id,x,y,sc,col,k,rhp){
    ...
    return `<g id="${id}" data-hand-x="${x+k.tx*sc*f}" data-hand-y="${y+k.ty*sc}" transform="translate(${x} ${y}) scale(${sc*f} ${sc})">
      ...
      <path d="M${k.hx-10} ${k.hy} L${k.sx-14+k.tw*3} ${k.sy+1-k.tw*4} Q${k.sx} ${k.sy-7} ${k.sx+14-k.tw*2} ${k.sy+1+k.tw*4} L${k.hx+10} ${k.hy} Z" fill="${col}" stroke="${shade(col,.65)}" stroke-width=".7"/>
      ${line(`M${k.hx-9} ${k.hy} h18`,"#232b2d",2)}
      <text x="${k.sx}" y="${k.sy+7}" text-anchor="middle" font-family="Noto Sans JP,sans-serif" font-weight="700" font-size="5.2" fill="#f0efe3" letter-spacing=".3" transform="translate(${k.sx} 0) scale(${f} 1) translate(${-k.sx} 0)">${esc(String((RV&&RV.c.pitP.name||"").replace(/\(.*\)/,"").split(/[・ 　]/)[0].slice(0,6)))}</text>
      <text x="${k.sx}" y="${k.sy+20}" text-anchor="middle" font-family="Arial,sans-serif" font-weight="bold" font-size="15" fill="#f0efe3" paint-order="stroke" stroke="#223143" stroke-width=".7" transform="translate(${k.sx} 0) scale(${f} 1) translate(${-k.sx} 0)">${esc(String(num??""))}</text>
  ...
  function umpFront(x, y, sc, up, fist){
    const rise = up * 14 * sc;
    const P = (dx, dy) => (x + dx * sc).toFixed(1) + " " + (y + dy * sc - rise).toFixed(1);
    const s_ = v => (v * sc).toFixed(1);
    const line = (d, c, w) => '<path d="' + d + '" stroke="' + c + '" stroke-width="' + s_(w) + '" fill="none" stroke-linecap="round" stroke-linejoin="round"/>';
    return '<g id="rv-ump">' +
      // 脚(開いてしゃがむ)
      line('M' + P(-9, -22) + ' L' + P(-16, -10) + ' L' + P(-14, 0), "#555a61", 7) + line('M' + P(9, -22) + ' L' + P(16, -10) + ' L' + P(14, 0), "#555a61", 7) +
      line('M' + P(-17, 0) + ' h7 M' + P(11, 0) + ' h7', "#151c23", 3) +
      // 胴(黒の上着)と腕(膝に手)
      '<path d="M' + P(-14, -24) + ' L' + P(-15, -54) + ' Q' + P(0, -62) + ' ' + P(15, -54) + ' L' + P(14, -24) + ' Z" fill="#2b2b30"/>' +
      (fist > 0 ? line('M' + P(13, -52) + ' L' + P(22, -60 - fist * 6) + ' L' + P(19, -74 - fist * 8), "#2b2b30", 5) + '<circle cx="' + (x + 19 * sc).toFixed(1) + '" cy="' + (y + (-74 - fist * 8) * sc - rise).toFixed(1) + '" r="' + s_(3.4) + '" fill="#e2b48a"/>'
                : line('M' + P(13, -52) + ' L' + P(19, -36) + ' L' + P(14, -22), "#2b2b30", 5) + '<circle cx="' + (x + 14 * sc).toFixed(1) + '" cy="' + (y - 22 * sc - rise).toFixed(1) + '" r="' + s_(3) + '" fill="#e2b48a"/>') +
      line('M' + P(-13, -52) + ' L' + P(-19, -36) + ' L' + P(-14, -22), "#2b2b30", 5) + '<circle cx="' + (x - 14 * sc).toFixed(1) + '" cy="' + (y - 22 * sc - rise).toFixed(1) + '" r="' + s_(3) + '" fill="#e2b48a"/>' +
      // 頭とマスク
      '<circle cx="' + (x).toFixed(1) + '" cy="' + (y - 68 * sc - rise).toFixed(1) + '" r="' + s_(7.5) + '" fill="#e2b48a"/>' +
      '<path d="M' + P(-7.5, -72) + ' a7.5 7.5 0 0 1 15 0" fill="#111"/>' +
      '<path d="M' + P(-7, -68) + ' l14 0 M' + P(-5.5, -64) + ' l11 0 M' + P(0, -74) + ' l0 12" stroke="#111" stroke-width="' + s_(1.2) + '" opacity=".8"/>' +
      '</g>';
  }
  ...
  function scoreBug(c){
    ...
    const sc = t => t === c.victim ? c.my : c.op;
    const ab = t => esc(String(t.name).replace(/[\s　]/g, "").slice(0, 3));
    const dots = (n, on, col) => Array.from({length:n}, (_, i) => '<circle cx="' + (i * 7) + '" cy="0" r="2.4" fill="' + (i < on ? col : "#2a3a3e") + '"/>').join("");
    const t = (x, y, s, col, txt, w) => '<text x="' + x + '" y="' + y + '" font-family="Noto Sans JP,Oswald,sans-serif" font-weight="' + (w || 700) + '" font-size="' + s + '" fill="' + col + '">' + txt + '</text>';
    return '<g transform="translate(262 236)">' +
      '<rect width="92" height="58" rx="2" fill="#0b1417" opacity=".9" stroke="#3a4c52" stroke-width=".8"/>' +
      '<rect width="92" height="13" fill="#1c2b31"/>' + t(6, 10, 8.5, "#ffd257", c.inn + "回" + (c.top ? "表" : "裏")) +
      '<rect x="0" y="13" width="54" height="22" fill="' + (away === c.victim ? "#243b48" : "#111b20") + '"/>' + t(5, 29, 12, "#fff", ab(away)) + '<text x="49" y="30" text-anchor="end" font-family="Oswald,sans-serif" font-weight="700" font-size="16" fill="#ffd257">' + sc(away) + '</text>' +
      '<rect x="0" y="35" width="54" height="22" fill="' + (home === c.victim ? "#243b48" : "#111b20") + '"/>' + t(5, 51, 12, "#fff", ab(home)) + '<text x="49" y="52" text-anchor="end" font-family="Oswald,sans-serif" font-weight="700" font-size="16" fill="#ffd257">' + sc(home) + '</text>' +
      '<g transform="translate(60 22)">' + t(0, 3, 8, "#cfe0d4", "S") + '<g transform="translate(11 0)">' + dots(2, 2, "#ffd257") + '</g></g>' +
      '<g transform="translate(60 36)">' + t(0, 3, 8, "#cfe0d4", "B") + '<g transform="translate(11 0)">' + dots(3, 3, "#6fe3a0") + '</g></g>' +
      '<g transform="translate(60 50)">' + t(0, 3, 8, "#cfe0d4", "O") + '<g transform="translate(11 0)">' + dots(2, c.outs, "#ff6b5b") + '</g></g>' +
      '</g>';
  }
  function stage(c, tr){
    const lefty = c.lefty, rhp = !(c.pitP && c.pitP.th === "左");
    const bx = lefty ? 150 : 262;                    // センターから見て右打者は右(三塁側)。左打者は投手と重ならない位置
    ...
    return stageOpen(300, "#0b1626") + stands(120) + boards() +
      ...
      '<ellipse cx="200" cy="282" rx="130" ry="24" fill="#8a5a34"/>' +
      ...
      '<path d="M186 290 l14 -7 l14 7 v5 h-28 z" fill="#f4f1e6" stroke="#333" stroke-width=".8"/>' +
      '<rect x="132" y="274" width="38" height="20" fill="none" stroke="#f4f1e6" stroke-opacity=".55" stroke-width="1.2"/><rect x="230" y="274" width="38" height="20" fill="none" stroke="#f4f1e6" stroke-opacity=".55" stroke-width="1.2"/>' +
      umpFront(CAT.x + (lefty ? -16 : 16), CAT.y - 8, 1.1, 0, 0) +
      catcherFront("rv-cat", CAT.x, CAT.y, CAT.sc, defCol, MITT0) +
      batter3d("rv-bat", B3_KF.stance, bx, 288, 52, batCol, !lefty) +
      ...
      // 左下: 球速(投球のあとに出る)
      '<text id="rv-cap" x="14" y="291" font-family="Oswald,sans-serif" font-weight="700" font-size="13" fill="#fff" letter-spacing="1" style="paint-order:stroke" stroke="#06110b" stroke-width="3"></text>' +
      lbl() + '</svg>';
  }
  function play(c){
    ...
    setLbl("満塁　フルカウント");
    const lefty=c.lefty,rhp=!(c.pitP&&c.pitP.th==="左"),bx=lefty?150:262;
    const defCol=teamCol(c.batting?c.opp:c.victim,"#4f8fe8"),batCol=teamCol(c.batting?c.victim:c.opp,"#e0a600");
    ...
    function f(ts){
      ...
      { const e=$r("rv-bat"); if(e) e.outerHTML=batter3d("rv-bat",b3Take(t-RELEASE,flight+25),bx,288,52,batCol,!lefty); }
      ...
      RV.timer=rvLater(()=>{
        const u=$r("rv-ump");if(u)u.outerHTML=umpFront(CAT.x+(lefty?-16:16),CAT.y-8,1.1,c.callIsStrike?1:0,c.callIsStrike?1:0);
        setLbl(c.callIsStrike?"球審の判定：ストライク":"球審の判定：ボール");
        RV.timer=rvLater(rvAsk,380);
      },280);
    }
    $r("rv-cap").textContent="";
...
window.rvDebug = {figSvg, P, lerpP, swingPose, takePose, managerFront, run, SCENES, pitcher3d, batter3d, B3_KF, b3Take, b3Swing, get rv(){ return RV; }};
})();
